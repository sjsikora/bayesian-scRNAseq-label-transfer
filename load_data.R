...
LD <- function() {
  cds_ref <<- load_monocle_objects("lineage_pap/cds_ref_LINEAGE")
  cds_qry <<- load_monocle_objects("lineage_pap/cds_qry_LINEAGE")
}
...
VD <- function() {
  ...
  transform_models_dir <<- 'lineage_pap/cds_ref_D_models'
  ref_column_ontology = list()
  k <<- 20
  verbose <<- FALSE
  nn_control <<- list()
...
T <- function() {
  ...
  cds_com <- combine_cds(list(cds_ref, cds_query),  keep_all_genes=TRUE, cell_names_unique=TRUE, keep_reduced_dims=TRUE)
