...
#' within ontogeny and affix it to the cell_data_set.
#' 
#' The ontogeny is found within ref_coldata. 
#' 
...
#' the ontogeny of the cell labels.
...
affix_ontogeny_labels <- function(
    cds_qry,
    ref_coldata,
    ref_column_names,
    query_column_name,
    ...
    , msg = "affix_ontogeny_labels: Invalid input")

    assertthat::assert_that(
        length(ref_column_names) == length(query_column_names)
    , msg = "affix_ontogeny_labels: ref_column_names and query_column_names must be the same length")

    assertthat::assert_that(
        all(ref_column_names %in% colnames(ref_coldata))
    , msg = "affix_ontogeny_labels: ref_column_names must be a subset of the column names of ref_coldata")

    assertthat::assert_that(
        query_column_name %in% colnames(colData(cds_qry))
    , msg = "affix_ontogeny_labels: query_column_name must be a column name of cds_qry")

    
    query_cells <- colData(cds_qry)[, query_column_name]
    ref_ontogeny <- as.data.frame(unique(ref_coldata[, ref_column_names]))

    NUMBER_OF_QUERY_CELLS <- length(query_cells)
    NUMBER_OF_LAYERS <- length(ref_column_names)

    #Turn ref_ontogeny into a list where the index is the most speficic label and the value is the row
    ref_ontogeny <- split(ref_ontogeny, ref_ontogeny[, NUMBER_OF_LAYERS])
    ...
    list_of_full_cell_paths <- lapply(query_cells, function(label) {
        ...
        return(as.vector(unlist(ref_ontogeny[[label]])))
    })
