...
assign_layer_labels <- function(cds_ref_D, cds_qry_D) {
  ...
  ontology <- list(
    ...
    c("L1.1", "L2.2", "NK", "NK"),
    c("L1.1", "L2.2", "B", "B"),
    c("L1.2", "L2.3", "L3.2", "Neu"),
    c("L1.2", "L2.3", "L3.2", "Baso"),
  ...
  for(i in 1:2) {
    ...
    for(j in 1:length(cell_labels)) df_list[[j]] <- ontology[[cell_labels[j]]]
  ...
  return(list_of_cds[[1]], list_of_cds[[2]])
